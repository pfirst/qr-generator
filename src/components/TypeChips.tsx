...
function Chip({ id, label, active, onPick }: { id: QRType; label: string; active: boolean; onPick: (t: QRType) => void }) {
  return (
    <button
      onClick={() => onPick(id)}
      className={
        'flex items-center gap-2 rounded-[13px] border px-3.5 py-2.5 text-[13px] font-bold transition ' +
        (active
          ? 'border-[#7c3aed] bg-[#ede9fe]/70 text-[#7c3aed] shadow-[0_2px_10px_rgba(124,58,237,0.16)]'
          : 'border-[#e6e7ee] bg-white text-[#6b7280] hover:border-[#c4b5fd] hover:text-[#111827]')
      }
    >
